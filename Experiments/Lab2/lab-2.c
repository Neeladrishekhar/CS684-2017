...
* Description:  Handle debounce key press by checking key press two times at a 10ms gap
...
* Description:  Set the next color in a cycle of Red then Green then Blue and repeat
...
* Description:  Set Timer0 to run at 100Hz(10ms) by enabling Timer0 Peripherals
...
* Description: The intterupt handler function for Timer0 as set up in timer0IntSetup()
